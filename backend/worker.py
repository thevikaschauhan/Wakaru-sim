...
def main() -> None:
    ...
    #
    # The #72 orphan-graph sweep is NO LONGER run here: it moved to a dedicated
    # Railway cron service (sweep.py), so this worker drains only the analyze
    # queue and needs neither the maintenance queue nor RQ's scheduler.
    worker = Worker(
        [ANALYZE_QUEUE_NAME],
